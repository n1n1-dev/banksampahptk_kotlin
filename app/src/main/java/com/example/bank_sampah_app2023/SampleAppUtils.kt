...
fun shareIntent(searchPlace: SearchPlace): Intent {
    val text = "${searchPlace.name}. " +
            "Alamat: ${searchPlace.address?.formattedAddress(SearchAddress.FormatStyle.Short) ?: "tidak diketahui"}. " +
            "Geo Koordinat: (lat=${searchPlace.coordinate.latitude()}, lon=${searchPlace.coordinate.longitude()})"
